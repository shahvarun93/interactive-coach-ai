...
•	Agentic AI: yes, when we build the guided multi-step interview flow.
	...
	•	POST /system-design/coach → takes stats + current session + topicMistakePatterns → generates summary + guidance.
	•	POST /system-design/system-design-study-plan → uses stats + resourcesByTopic → generates a multi-step plan.
